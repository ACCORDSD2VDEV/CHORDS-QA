...
tab1_cl <- run_query(paste0("select count(*) as nrows
...
# set up empty container for alert messages
QA_Alert_Messages <- data.frame(#'Alert Code'=character(),
                 'Alert Message'=character(), stringsAsFactors = FALSE)

# populate as:  QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = c(missingTableMessage)
...
# alerts for demographics
missDobPct <- 100*tab2_demo_null[,'BIRTH_DATE']/tab1_dem$nrows

if(missDobPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The demographics table has ',round(missDobPct,1),'% rows missing DOB')}

missGenderPct <- 100*tab2_demo_null[,'GENDER']/tab1_dem$nrows

if(missGenderPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The demographics table has ',round(missGenderPct,1),'% rows missing gender')}

missPLangPct <- 100*tab2_demo_null[,'PRIMARY_LANGUAGE']/tab1_dem$nrows

if(missPLangPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The demographics table has ',round(missPLangPct,1),'% rows missing primary language')}

missRace1Pct <- 100*tab2_demo_null[,'RACE1']/tab1_dem$nrows

if(missRace1Pct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The demographics table has ',round(missRace1Pct,1),'% rows missing race 1')}

missHispPct <- 100*tab2_demo_null[,'HISPANIC']/tab1_dem$nrows

if(missHispPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The demographics table has ',round(missHispPct,1),'% rows missing hispanic flag')}
...
# alerts for the encounter table
missAdatePct <- 100*tab2_enc_null[,'ADATE']/tab1_enc$nrows

if(missAdatePct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missAdatePct,1),'% rows missing ADATE')}

missDdatePct <- 100*tab2_enc_null[,'DDATE']/tab1_enc$nrows

if(missDdatePct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missDdatePct,1),'% rows missing DDATE')}

missProvPct <- 100*tab2_enc_null[,'PROVIDER']/tab1_enc$nrows

if(missProvPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missProvPct,1),'% rows missing provider')}

missEncTypePct <- 100*tab2_enc_null[,'ENCOUNTER_TYPE']/tab1_enc$nrows

if(missEncTypePct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missEncTypePct,1),'% rows missing encounter type')}

missEncSubTypePct <- 100*tab2_enc_null[,'ENCOUNTER_SUBTYPE']/tab1_enc$nrows

if(missEncSubTypePct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missEncSubTypePct,1),'% rows missing encounter sub type')}

missFacPct <- 100*tab2_enc_null[,'FACILITY']/tab1_enc$nrows

if(missFacPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missFacPct,1),'% rows missing facility')}

missDeptPct <- 100*tab2_enc_null[,'DEPARTMENT']/tab1_enc$nrows

if(missDeptPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The encounter table has ',round(missDeptPct,1),'% rows missing department')}
...
# alerts for census location
missGeoCdPct <- 100*tab2_cl_null[,'GEOCODE']/tab1_cl$nrows

if(missGeoCdPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The census location table has ',round(missGeoCdPct,1),'% rows missing geocode')}
...
# alerts for the diagnosis table
missEncIDPct <- 100*tab2_dx_null[,'ENC_ID']/tab1_dx$nrows

if(missEncIDPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The diagnosis table has ',round(missEncIDPct,1),'% rows missing encounter ID')}

missAdateDXPct <- 100*tab2_dx_null[,'ADATE']/tab1_dx$nrows

if(missAdateDXPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The diagnosis table has ',round(missAdateDXPct,1),'% rows missing ADATE')}

missDXDXPct <- 100*tab2_dx_null[,'DX']/tab1_dx$nrows

if(missDXDXPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The diagnosis table has ',round(missDXDXPct,1),'% rows missing diagnosis')}

missDXCodeTypeDXPct <- 100*tab2_dx_null[,'DX_CODETYPE']/tab1_dx$nrows

if(missDXCodeTypeDXPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The diagnosis table has ',round(missDXCodeTypeDXPct,1),'% rows missing diagnosis code type')}

missPrinDXPct <- 100*tab2_dx_null[,'PRINCIPAL_DX']/tab1_dx$nrows

if(missPrinDXPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The diagnosis table has ',round(missPrinDXPct,1),'% rows missing principal diagnosis flag')}

missPrimDXPct <- 100*tab2_dx_null[,'PRIMARY_DX']/tab1_dx$nrows

if(missPrimDXPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The diagnosis table has ',round(missPrimDXPct,1),'% rows missing primary diagnosis flag')}
...
missEncTypeVSPct <- 100*tab2_vs_null[,'ENCTYPE']/tab1_vs$nrows

if(missEncTypeVSPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The vital signs table has ',round(missEncTypeVSPct,1),'% rows missing encounter type')}

missEncIdVSPct <- 100*tab2_vs_null[,'ENC_ID']/tab1_vs$nrows

if(missEncIdVSPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The vital signs table has ',round(missEncIdVSPct,1),'% rows missing encounter ID')}
...
#alerts for the benefit table
missBenTypeBenPct <- 100*tab2_benefit_null[,'BENEFIT_TYPE']/tab1_ben$nrows

if(missBenTypeBenPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The benefit table has ',round(missBenTypeBenPct,1),'% rows missing benefit type')}

missBenCatBenPct <- 100*tab2_benefit_null[,'BENEFIT_CAT']/tab1_ben$nrows

if(missBenCatBenPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The benefit table has ',round(missBenCatBenPct,1),'% rows missing benefit category')}

missBenDateBenPct <- 100*tab2_benefit_null[,'BENEFIT_DATE']/tab1_ben$nrows

if(missBenDateBenPct>1) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The benefit table has ',round(missBenDateBenPct,1),'% rows missing benefit date')}

missEncIdBenPct <- 100*tab2_benefit_null[,'ENC_ID']/tab1_ben$nrows

if(missEncIdBenPct>10) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = paste0('The benefit table has ',round(missEncIdBenPct,1),'% rows missing encounter ID')}
...
encTmp1 <-   sqlQuery(tempCon1,
                  paste0('SET NOCOUNT ON; 
                         SELECT d.*, demo.RACE1, demo.HISPANIC
                         INTO #encTmp1 
                         FROM (
                            SELECT a.PERSON_ID, 
                         ...
                         inner join (
                            ...
                            from  ',demographics,') as b on a.person_id = b.person_id
                            group by a.PERSON_ID) as d
                         inner join ',demographics,' demo on demo.PERSON_ID = d.PERSON_ID'
                        )
                  )

tab7.11_rowCnt <-sqlQuery(tempCon1, paste0("
    SELECT lastEnc.year
    	,lastEnc.rowCnt rowCnt
    	,ISNULL(missingRaceHispanic.rowCnt, 0) missingRowCnt
    FROM (
    	SELECT YEAR(lastEnc) AS year
    		,COUNT(*) AS rowCnt
    	FROM #encTmp1
    	GROUP BY YEAR(lastEnc)
    	) lastEnc
    LEFT JOIN (
    	SELECT YEAR(lastEnc) AS year
    		,COUNT(*) AS rowCnt
    	...
    	WHERE Race1 IN ('UN')
    		AND Hispanic IN ('U')
    	GROUP BY YEAR(lastEnc)
    	) missingRaceHispanic ON missingRaceHispanic.year = lastEnc.year
    ORDER BY lastEnc.year")
)
...
```{r validationMessage Output, echo = F}
if(nrow(QA_Alert_Messages)==0) {QA_Alert_Messages[nrow(QA_Alert_Messages) + 1,] = 'No specific alerts noted'}
...
```{r tab6.6_disp output, echo = F, include=T}
